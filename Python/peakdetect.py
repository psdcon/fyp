    ...
    for index, (x, y) in enumerate(zip(x_axis[:-lookahead],
        ...
        if y < mx-delta and mx != np.Inf:
            ...
            if y_axis[index:index+lookahead].max() < mx:
                itemIndex = np.where(x_axis == mxpos)
                max_peaks.append({"x": mxpos, "y": mx, "index": itemIndex[0][0]})
                dump.append(True)
        ...
        if y > mn+delta and mn != -np.Inf:
            ...
            if y_axis[index:index+lookahead].min() > mn:
                itemIndex = np.where(x_axis == mnpos)
                min_peaks.append({"x": mnpos, "y": mn, "index": itemIndex[0][0]})
                dump.append(False)
